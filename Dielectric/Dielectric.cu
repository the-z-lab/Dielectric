			...
			Scalar prefac ) // precomputed term (2*xi^2/PI)^(3/2)*1/(eta.x*eta.y*eta.z)^(1/2)
{
	...
	Scalar3 Sj = d_dipole[idx];
	//printf("[Before] spread dipole: group_idx = %d, Sj = d_dipole[group_idx] = (%f, %f, %f) \n", group_idx, d_dipole[group_idx].x, d_dipole[group_idx].y, d_dipole[group_idx].z);
	//printf("[Changed] spread dipole: idx = %d, Sj = d_dipole[idx] = (%f, %f, %f) \n", idx, Sj.x, Sj.y, Sj.z);
			...
			Scalar prefac ) // precomputed term (2*xi^2/PI)^(3/2)*1/(eta.x*eta.y*eta.z)^(1/2)
{
	...
	//printf("[Befofe] spread: group_idx = %d, Sj = d_dipole[group_idx] = (%f, %f, %f) \n", group_idx, d_dipole[group_idx].x, d_dipole[group_idx].y, d_dipole[group_idx].z);
				...
				Scalar prefac) // precomputed term (2*xi^2/PI)^(3/2)*1/(eta.x*eta.y*eta.z)^(1/2)*gridh.x*gridh.y*gridh.z
{
	...
	if (thread_offset == 0){
		...
		d_output[idx] = output[0];
		//printf("[Changed] contract: idx = %d, d_output[idx] = (%f, %f, %f) \n", idx, output[0].x, output[0].y, output[0].z);
	}
}
				...
				Scalar prefac ) // precomputed term (2*xi^2/PI)^(3/2)*1/(eta.x*eta.y*eta.z)^(1/2)*gridh.x*gridh.y*gridh.z
{
	...
	//printf("[Befofe] contract force: group_idx = %d, Si = d_dipole[group_idx] = (%f, %f, %f) \n", group_idx, d_dipole[group_idx].x, d_dipole[group_idx].y, d_dipole[group_idx].z);
					...
					const unsigned int *d_head_list) // index used to access elements of the neighbor list
{
	...
	// Ensure that current particle is within the group
  	if (group_idx < group_size) {
		...
		// Global ID of current particle
		unsigned int idx = d_group_members[group_idx];
		//printf("[Original] real space charge: group_idx = %d, idx = d_group_members[group_idx] = %d \n", group_idx, idx);

		// Get the wave space contribution to E0 - M_Eq * q
  		Scalar3 Eq = d_Eq[idx];
		//printf("[Before] real space charge: group_idx = %d, Eq = d_Eq[group_idx] = (%f, %f, %f) \n", group_idx, d_Eq[group_idx].x, d_Eq[group_idx].y, d_Eq[group_idx].z);
		//printf("[Changed] real space charge: idx = %d, Eq = d_Eq[idx] = (%f, %f, %f) \n", idx, Eq.x, Eq.y, Eq.z);
		...
		// Loop over neighbors
			...
			if ( neigh_group_idx != -1 ) {
				...
				// Add neighbor contribution if it is within the real space cutoff radius
					...
					//printf("[Original] real space charge: neigh_group_idx = %d, qj = d_charge[neigh_group_idx] = %f \n", neigh_group_idx, qj);
		...
		d_Eq[idx] = E0 - Eq;
		//printf("[Before] real space charge: group_idx = %d, d_Eq[group_idx] = (%f, %f, %f) \n", group_idx, d_Eq[group_idx].x, d_Eq[group_idx].y, d_Eq[group_idx].z);
		//printf("[Changed] real space charge: idx = %d, d_Eq[idx] = (%f, %f, %f) \n", idx, d_Eq[idx].x, d_Eq[idx].y, d_Eq[idx].z);
					...
					Scalar selfcoeff) // coefficient of the self term
{
	...
	// Ensure that current particle is within the group
  	if (group_idx < group_size) {
		...
		// Global ID of current particle
		unsigned int idx = d_group_members[group_idx];
		//printf("[Original] real space field dipole: group_idx = %d, idx = d_group_members[group_idx] = %d \n", group_idx, idx);

		// Get the wave space contribution to M_ES * S
  		Scalar3 ES = d_ES[idx];
		//printf("[Before] real space field dipole: group_idx = %d, ES = d_ES[group_idx] = (%f, %f, %f) \n", group_idx, d_ES[group_idx].x, d_ES[group_idx].y, d_ES[group_idx].z);
		...
		Scalar3 Si = d_dipole[idx];
		Scalar lambda_p = d_conductivity[idx];
		//printf("[Before] real space field dipole: group_idx = %d, Si = d_dipole[group_idx] = (%f, %f, %f) \n", group_idx, d_dipole[group_idx].x, d_dipole[group_idx].y, d_dipole[group_idx].z);
		//printf("[Before] real space field dipole: group_idx = %d, lambda_p = d_conductivity[group_idx] = %f \n", group_idx, d_conductivity[group_idx]);
		...
		d_ES[idx] = ES;
		//printf("[Before] real space field dipole: group_idx = %d, d_ES[group_idx] = (%f, %f, %f) \n", group_idx, d_ES[group_idx].x, d_ES[group_idx].y, d_ES[group_idx].z);
					...
					const unsigned int *d_head_list) // index used to access elements of the neighbor list
{
	...
	// Ensure that current particle is within the group
  	if (group_idx < group_size) {
		...
		// Global ID of current particle
		unsigned int idx = d_group_members[group_idx];
		//printf("[Original] real space force charge: group_idx = %d, idx = d_group_members[group_idx] = %d \n", group_idx, idx);

		// Get the reciprocal contribution to the force
  		Scalar4 F4 = d_force[idx];
		Scalar3 F = make_scalar3(F4.x, F4.y, F4.z);
		//printf("[Original] real space force charge: idx = %d, F4 = d_force[idx] = (%f, %f, %f) \n", idx, F4.x, F4.y, F4.z);

		// Charge and dipole of current particle
		Scalar qi = d_charge[idx];
		//printf("[Before] real space force charge: group_idx = %d, qi = d_charge[group_idx] = %f \n", group_idx, d_charge[group_idx]);
		//printf("[Changed] real space force charge: idx = %d, qi = d_charge[idx] = %f \n", idx, d_charge[idx]);
		...
		// Loop over neighbors
			...
			if ( neigh_group_idx != -1 ) {
				...
				// Add neighbor contribution if it is within the real space cutoff radius
					...
					// Charge of neighbor particle
					Scalar qj = d_charge[neigh_group_idx];
					//printf("[Original] real space force charge: neigh_group_idx = %d, qj = d_charge[neigh_group_idx] = %f \n", neigh_group_idx, qj);
		...
		d_force[idx] = F4;
		//printf("[Original] real space force charge: idx = %d, d_force[idx] = F4 = (%f, %f, %f) \n", idx, F4.x, F4.y, F4.z);
	}
}
					...
					const unsigned int *d_head_list) // index used to access elements of the neighbor list
{
	...
	// Ensure that current particle is within the group
  	if (group_idx < group_size) {
		...
		// Global ID of current particle
		unsigned int idx = d_group_members[group_idx];
		//printf("[Original] real space force: group_idx = %d, idx = d_group_members[group_idx] = %d \n", group_idx, idx);

		// Get the reciprocal contribution to the force
  		Scalar4 F4 = d_force[idx];
		Scalar3 F = make_scalar3(F4.x, F4.y, F4.z);
		//printf("[Original] real space force: idx = %d, F4 = d_force[idx] = (%f, %f, %f) \n", idx, F4.x, F4.y, F4.z);
		...
		//printf("[Befofe] real space force: group_idx = %d, Si = d_dipole[group_idx] = (%f, %f, %f) \n", group_idx, d_dipole[group_idx].x, d_dipole[group_idx].y, d_dipole[group_idx].z);
		...
		// Loop over neighbors
			...
			if ( neigh_group_idx != -1 ) {
				...
				// Add neighbor contribution if it is within the real space cutoff radius
					...
					Scalar qj = d_charge[neigh_group_idx];
					Scalar3 Sj = d_dipole[neigh_group_idx];
					//printf("[Original] real space force: neigh_group_idx = %d, qj = d_charge[neigh_group_idx] = %f \n", neigh_group_idx, qj);
		...
		F4 = make_scalar4(F.x, F.y, F.z, 0.0);
		d_force[idx] = F4;
		//printf("[Original] real space force: idx = %d, d_force[idx] = F4 = (%f, %f, %f) \n", idx, F4.x, F4.y, F4.z);
	}
}
